...
use crate::{
    ...
    utils::{
        ...
        SelectOption,
        handle_uncommitted_changes_before_action, prompt_and_execute_pull,
    },
};
use dialoguer::Select; // Reset のモード選択に使用
...
#[derive(Parser, Debug)]
pub struct ResetArgs {} // Reset 用の引数構造体
...
pub fn git_switch(_args: SwitchArgs) -> Result<()> {
    // 1. 未コミット変更の処理
    if !handle_uncommitted_changes_before_action("switch", &GitCommand)? {
        // Save, Create, Cancel が選択された場合はここで終了
        return Ok(());
    }
    // 変更がなかった、または Reset された場合のみ続行
    ...
    let target_input = match prompt_fuzzy_select("切り替えたいブランチ:", &branch_options)? {
        ...
        None => { println!("ブランチ選択がキャンセルされました。"); return Ok(()); }
    };

    // 3. ブランチ切り替え処理
    let resolved = resolve_branch_input(&target_input, &GitCommand)?;
    let checkout_branch_name = resolved.local_candidate_name.clone();
    let mut switched = false;

    if resolved.local_exists && resolved.local_candidate_name == target_input {
        GitCommand::checkout(&checkout_branch_name).with_context(|| format!("ブランチ '{}' への切り替え失敗", checkout_branch_name))?;
        println!("ブランチ '{}' へ切り替えました。", checkout_branch_name.cyan());
        switched = true;
        ...
        if prompt_confirm(&prompt_msg)? {
            GitCommand::checkout(&checkout_branch_name).with_context(|| format!("ブランチ '{}' の作成と切り替え失敗", checkout_branch_name))?;
         ...
         bail!("エラー: ブランチ '{}' は見つかりません...", target_input.red());
    };

    // 4. 自動プル確認・実行
    if switched {
        prompt_and_execute_pull("origin", &checkout_branch_name, &GitCommand)?;
    }
...
// --- Reset コマンドハンドラ ---
pub fn git_reset(_args: ResetArgs) -> Result<()> {
    println!("{}", "どのモードでリセットしますか？ (対象: HEAD~ 直前のコミット)".yellow());
    let modes = vec![
        SelectOption{ display: "soft: コミットのみ取り消し (変更はステージングに残る)".to_string(), value: "--soft".to_string()},
        SelectOption{ display: "mixed (デフォルト): コミットとステージングを取り消し (変更は作業ディレクトリに残る)".to_string(), value: "--mixed".to_string()},
        SelectOption{ display: "hard: コミット・ステージング・作業ディレクトリの変更をすべて破棄 (危険！)".to_string(), value: "--hard".to_string()},
        SelectOption{ display: "cancel: 操作を中止".to_string(), value: "cancel".to_string()},
    ];
    
    // 選択肢が固定で少ないので Select を使う
    let selected_mode_opt = match Select::with_theme(&ColorfulTheme::default())
        .with_prompt("リセットモードを選択:")
        .items(&modes.iter().map(|opt| opt.display.as_str()).collect::<Vec<_>>())
        .default(1) // mixedをデフォルトに
        .interact_opt()? { // Option<usize>
            Some(idx) => Some(modes[idx].value.clone()),
            None => None
        };

    match selected_mode_opt {
        Some(mode) if mode == "cancel" => {
            println!("リセット操作をキャンセルしました。");
            Ok(())
        }
        Some(mode) => {
            if mode == "--hard" {
                println!("{}", "警告: --hard モードは作業ディレクトリの変更を完全に失います！".red().bold());
                if !prompt_confirm("本当によろしいですか？")? {
                    println!("リセット操作をキャンセルしました。");
                    return Ok(());
                }
            }
            // GitCommand::reset_head を呼び出す
            GitCommand::reset_head(Some(mode.as_str())).context("git reset の実行に失敗")?;
            println!("HEAD~ ({}) でリセットを実行しました。", mode);
            Ok(())
        }
        None => { // 選択がキャンセルされた場合 (Escなど)
            println!("リセット操作がキャンセルされました。");
            Ok(())
        }
    }
}